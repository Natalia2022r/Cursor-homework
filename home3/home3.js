...
const calcResult = (salary, taxRate) =>
    salary - salary * (parseFloat(taxRate) / 100);
...
document.getElementById("result8").innerHTML = `<div ><p>function getRandomPassword(length = 8) => ${getRandomPassword((length = 8)
)}</p></div>`;
document.getElementById("result81").innerHTML = `<div ><p>function getRandomPassword(length = 6) => ${getRandomPassword((length = 6)
)}</p></div>`;
